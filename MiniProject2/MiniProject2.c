...
void graph2CNFAHunt(int k, char* path) {
...
int main(int argc, char *argv[]){
    ...
    char path[100] = "../TestData/datagraph/";
    strcat(path, fileName);
    printf("path: %s, k = %d\n", path, k);
    readGraph(path);

    char path1[100] = "../TestData/graph2cnf/";
    strcat(path1, fileName);
    strcat(path1, "2CNF.txt");
    graph2CNFAHunt(k, path1);

    struct SATProblem problem = readSATProblem(path1);
    struct Affectation affectation;
    affectation.affec = malloc(problem.variables*sizeof(int));
    for(int i = 0; i<problem.variables; i++) {
        affectation.affec[i] = 0;
    }
    //printf("%d", verifySAT(problem, affectation));
    begin = clock();
